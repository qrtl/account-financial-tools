...
class TestAccountMoveGroupRestriction(TransactionCase):
    ...
    def test_basic_user_cannot_see_restricted_moves(self):
        ...
        visible_public = Move_basic.search([("id", "=", self.move_public.id)])
        visible_restricted = Move_basic.search([("id", "=", self.move_restricted.id)])
        visible_mixed = Move_basic.search([("id", "=", self.move_mixed.id)])
        self.assertEqual(
            visible_public,
            self.move_public,
